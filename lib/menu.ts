 ...
 * 파란 메뉴바와 좌측 사이드바라는 원본 틀은 유지하되, 업종 전환(대부업 → 렌터카)에 맞춰
 * 메뉴 구성을 교체했다. 원본의 "항상 펼쳐진 2행 서브메뉴"는 산만해서 드롭다운으로 바꿨다.
 *
 * [업종 전환 변경 내역]
...
export type MenuSection = MenuItem & {
  ...
  sidebarTitle: string;
  /** 하위 메뉴 (좌측 사이드바 + 상단 메뉴 드롭다운에 사용) */
  children: MenuItem[];
  /** (참고용) 원본에서 상단 2행에 하위 메뉴를 노출하던 섹션 표시. 현재 헤더는 children 개수로 판단한다 */
